...
namespace Rogueskiv.Ux.Renderers
{
    class TimerRenderer : TextRenderer<TimerComp>
    {
        ...
        public TimerRenderer(UxContext uxContext, IGameContext gameContext, IntPtr font)
            : base(uxContext, font) =>
        ...
        protected override void Render(IEntity entity, TimerComp timerComp, float interpolation)
        {
            ...
            Render("IGT: " + new TimeSpan(inGameTicks).ToString(format), TEXT_COLOR, position);

            position = position.Add(y: HEIGHT);
            Render("RTA: " + realTime.ToString(format), TEXT_COLOR, position);
        }
